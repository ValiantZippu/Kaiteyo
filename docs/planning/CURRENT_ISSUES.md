...
| # | Item | Category | Notes |
|---|------|----------|-------|
| 8 | OCR is Tesseract-dependent; missing-engine UX is a hint, not a guided setup | KNOWN ISSUE | hardening planned — see `docs/architecture/ocr.md` (provider abstraction, guided setup, caching) |
...
| 13 | **Master architecture pass (2026-08-30)** — unified documentation now exists: 30-point audit (`docs/planning/MASTER_AUDIT_2026.md`) + Core/content model + per-subsystem master specs (Media/Subtitles/Browser/Reading/OCR/Dictionary/Mining/Anki/Events/Platforms/Stats/Library/Browse/Home/Game/Sync/Security/Navigation/Window/Debug/Design) + integrations (Yomitan/ASBPlayer/Jidoujisho/Chromium/AniList/MAL) + phased Roadmap + Testing Plan | DONE | Source-only documentation pass — no code changes; `gradlew :desktopApp:compileKotlinJvm` unchanged; next: Phase 0 service contracts + ADR-0017 |
