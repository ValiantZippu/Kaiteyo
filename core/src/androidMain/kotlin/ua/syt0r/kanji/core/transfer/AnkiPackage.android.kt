...
actual class AnkiPackage {
    ...
    actual fun write(cards: List<KaiteyoCard>, deckName: String): Result<ByteArray> {
        return Result.failure(UnsupportedOperationException("APKG export not implemented on Android yet"))
    }

    actual fun read(bytes: ByteArray): Result<List<KaiteyoCard>> {
        return Result.failure(UnsupportedOperationException("APKG import not implemented on Android yet"))
    }
